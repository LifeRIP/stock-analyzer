<template>
  <div>
    <div class="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
      ...
      <div class="flex items-center space-x-2">
        ...
        <select 
          id="page-size" 
          v-model="pageSize" 
          class="border rounded-md px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
    ...
    <div v-else-if="stocks.error" class="text-center my-8 text-gray-500">
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200 table-fixed">
            <thead class="bg-gray-50">
              <tr>
                <th scope="col" class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-[10%]">Ticker</th>
                <th scope="col" class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-[20%]">Company</th>
                <th scope="col" class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-[20%]">Broker</th>
                <th scope="col" class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-[15%]">Action</th>
                <th scope="col" class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-[15%]">Rating</th>
                <th scope="col" class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-[12%]">Target</th>
                <th scope="col" class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-[8%]">Date</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="stock in stocks" :key="stock.id" class="hover:bg-gray-50 cursor-pointer" @click="$emit('stock-click', stock.ticker)">
                <td class="px-3 py-3 whitespace-nowrap">
                  <div class="font-medium text-green-600">{{ stock.ticker }}</div>
                </td>
                <td class="px-3 py-3">
                  <div class="text-sm text-gray-900 truncate" :title="stock.company">{{ stock.company }}</div>
                </td>
                <td class="px-3 py-3">
                  <div class="text-sm text-gray-500 truncate" :title="stock.brokerage">{{ stock.brokerage }}</div>
                </td>
                <td class="px-3 py-3">
                  <div class="text-sm text-gray-500 truncate" :title="stock.action">{{ stock.action }}</div>
                </td>
                <td class="px-3 py-3">
                  <div class="text-sm" :class="getRatingClass(stock.rating_from, stock.rating_to)">
                    <template v-if="stock.rating_from && stock.rating_from !== stock.rating_to">
                      <span class="text-gray-500">{{ stock.rating_from }}</span> → {{ stock.rating_to }}
                    </template>
                    <template v-else>
                      {{ stock.rating_to || 'N/A' }}
                    </template>
                ...
                <td class="px-3 py-3">
                  <div class="text-sm" :class="getTargetClass(stock.target_from, stock.target_to)">
                    <template v-if="stock.target_from !== stock.target_to">
                      <span class="text-gray-500">{{ stock.target_from }}</span> → {{ stock.target_to }}
                    </template>
                    <template v-else>
                      {{ stock.target_to }}
                    </template>
